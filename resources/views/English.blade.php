...
<body>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const englishRoute = "{{ route('English') }}";
            ...
            function loadSubject(url) {
                ...
                .then(html => {
                    ...
                    const content = doc.querySelector('#lessonSection');

                    if (content) {
                    lessonSection.classList.remove('d-none');
                    lessonSection.innerHTML = content.innerHTML;
                    } else {
                    console.warn('No #lessonSection found in fetched page.');
                    }
                })
                .catch(err => console.error('Error loading subject:', err));
            }

            // 🟢 When "Curriculum & Lessons" clicked, show English by default
            const curriculumLink = document.getElementById('curriculumLink');
            curriculumLink.addEventListener('click', (e) => {
                // Prevent only navigation (not collapse toggle)
                e.preventDefault();
                // Show English automatically
                loadSubject(englishRoute);
                // Also ensure dropdown expands
                const subjectList = document.getElementById('subject-list');
                const bsCollapse = bootstrap.Collapse.getOrCreateInstance(subjectList);
                bsCollapse.show();
            });
            ...
            document.querySelectorAll('.sidebar-link.nav-link').forEach(link => {
                ...
                e.preventDefault();
                loadSubject(link.getAttribute('href'));
                });
            });
        });
    </script>
